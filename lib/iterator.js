...
function statIterator (drive, path, opts) {
  ...
  function next (cb) {
    ...
    stack[0].iterator.next((err, node) => {
      ...
      const trie = node[MountableDWebTrie.Symbols.TRIE]
      const mount = node[MountableDWebTrie.Symbols.MOUNT]
      const innerPath = node[MountableDWebTrie.Symbols.INNER_PATH]
